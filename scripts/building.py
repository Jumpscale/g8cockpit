...
def build(host, sshkey):
    ...
    container.apps.portal.install(start=False, minimal=True)
    container.mongodb.build(start=False)
    container.influxdb.build(start=False)
    container.grafana.build(start=False)
